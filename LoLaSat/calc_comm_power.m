function [time,thrust_power] = calc_comm_power(p_idle,p_half,p_full, d_h, d_f, n, m, t0, t_n, t_m)
...
tpulse=[0, 1,     1+d_h, 2+d_h, 2+d_h+d_f,d_h+d_f+3, d_h+d_f+3+t_n];
ppulse=[0, p_half,p_half,p_full,p_full,   0, 0];
%plot(tpulse,ppulse)
...
thrust_power = [0, 0];
for i=1:n
    time = horzcat( time, t0+(i-1)*(t_n+d_h+d_f+3)+tpulse );
